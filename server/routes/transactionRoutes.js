...
//         // 2) LOOP THROUGH ITEMS AND FETCH LATEST PRICE
//         for (let item of loadout.items) {

//             const latestPrice = await Rate.findOne({
...
//             const price = latestPrice ? latestPrice.rate : 0;
...
// router.post("/transaction/settlement", async (req, res) => {
//     try {
//         const { salesmanCode, date, trip } = req.body;
//         const selectedDate = new Date(date);
...
//         // 3) LOOP THROUGH LOADOUT ITEMS
//         for (let item of loadout.items) {

//             // find matching item in loadin
